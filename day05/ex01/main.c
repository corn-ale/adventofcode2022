...
int	fakemain()
{
	...
	while (instr)
	{
		...
		while (nums[0] > 0)		// move nums[0] number of crates
		{						
			// go to stacks nums[1]
			for (ww = 0; arrayOfStacks[nums[1] - 1][ww + 1] != '\0'; ww++)
				;				// find last crate before null
			Crate = arrayOfStacks[nums[1] - 1][ww];
			// printf("[%c] ", Crate);					// take crate
			arrayOfStacks[nums[1] - 1][ww] = '\0';
			for (yy = 0; arrayOfStacks[nums[2] - 1][yy] != '\0'; yy++)
				;
			arrayOfStacks[nums[2] - 1][yy] = Crate;
			nums[0]--;
		}
		// for (int tmp = 0; arrayOfStacks[tmp]; tmp++)
		// 	printf("stack %d: %s\n", tmp+1, arrayOfStacks[tmp]);
